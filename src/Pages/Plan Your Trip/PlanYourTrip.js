...
import { pytPropertiesFetch, PYTSectionFetch } from '../../redux/pagesSlice';

const PlanYourTrip = () => {
    const dataPyt = useSelector((state) => state.page.dataPyt);
    const propertiesPytData = useSelector((state) => state.page.propertiesPytData);
    const dispatch = useDispatch()

    useEffect(() => {
      dispatch(PYTSectionFetch())
      dispatch(pytPropertiesFetch())
    }, [])
    return (
        <div>
            ...
            <div className="ml-[25px] flex flex-col flex-wrap gap-y-4 max-w-[1560px] pt-4 ">
                ...
                <h1 className="absolute bottom-[650px] font-medium left-[400px] text-[28px] text-white">Plan Your Trip Page</h1>
